...
```{r}
midwest %>% ggplot(aes(x=area, y=log(poptotal))) + geom_point(alpha=.5) + geom_smooth(method="lm")
```
...
```{r}
midwest %>% ggplot(aes(x=log((100-percwhite)), y=percbelowpoverty)) + geom_point(alpha=.5) + geom_smooth(method="lm")
```
...
```{r}
plotdat <- midwest %>% mutate(lognonwhite = log(100 - percwhite)) %>% 
    group_by(state) %>% 
...
```{r}
testdat <- midwest %>% mutate(lognonwhite = log(100 - percwhite)) %>% 
